...
run_root() {
  if [ "$(id -u 2>/dev/null || echo 1)" = "0" ]; then
    ...
    echo "Root privileges are required to install Midnight Commander. Run as root or configure passwordless sudo/doas for the ShellOrchestra account." >&2
    exit 1
  fi
}
run_root apt-get update >&2
run_root env DEBIAN_FRONTEND=noninteractive apt-get -y install mc >&2
install_shellorchestra_mc_skin
