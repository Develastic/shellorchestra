...
if ($env:SHELLORCHESTRA_DRY_RUN -eq '1' -or $env:SHELLORCHESTRA_CONFIRMED -ne '1') {
  ...
  [Console]::Out.Write((([ordered]@{ ok = $true; dry_run = $true; manager = $manager; operation = 'remove'; package = $packageName }) | ConvertTo-Json -Compress))
  exit 0
}
...
Invoke-ShellOrchestraWingetAction -WingetPath $wingetPath -Arguments "uninstall --source winget --id $packageName --disable-interactivity" -FailurePrefix 'winget could not remove the selected package'
[Console]::Out.Write((([ordered]@{ ok = $true; manager = $manager; operation = 'remove'; package = $packageName }) | ConvertTo-Json -Compress))
