...
if [ "${SHELLORCHESTRA_DRY_RUN:-0}" = "1" ] || [ "${SHELLORCHESTRA_CONFIRMED:-0}" != "1" ]; then
  printf '{"ok":true,"manager":"apt","dry_run":true}\n'
  exit 0
fi
shellorchestra_log=$(mktemp)
trap 'rm -f "$shellorchestra_log"' EXIT
run_root apt-get update >>"$shellorchestra_log" 2>&1
run_root env DEBIAN_FRONTEND=noninteractive apt-get -y upgrade >>"$shellorchestra_log" 2>&1
updated_count=$(awk '/ upgraded,/{print $1; exit}' "$shellorchestra_log")
updated_packages_json=$(awk 'BEGIN{flag=0} /^The following packages will be upgraded:/{flag=1; next} flag && /^[[:space:]]/{for(i=1;i<=NF;i++) print $i; next} flag && NF==0{flag=0}' "$shellorchestra_log" | sort -u | json_string_list)
finish_json apt "$updated_count" "$updated_packages_json"
