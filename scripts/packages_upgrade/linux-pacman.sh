...
if [ "${SHELLORCHESTRA_DRY_RUN:-0}" = "1" ] || [ "${SHELLORCHESTRA_CONFIRMED:-0}" != "1" ]; then
  printf '{"ok":true,"manager":"pacman","dry_run":true}\n'
  exit 0
fi
shellorchestra_log=$(mktemp)
trap 'rm -f "$shellorchestra_log"' EXIT
run_root pacman -Syu --noconfirm >>"$shellorchestra_log" 2>&1
updated_packages_json=$(awk '/^upgrading /{print $2}' "$shellorchestra_log" | sort -u | json_string_list)
updated_count=$(awk '/^upgrading /{count++} END{if(count>0) print count; else print 0}' "$shellorchestra_log")
finish_json pacman "$updated_count" "$updated_packages_json"
