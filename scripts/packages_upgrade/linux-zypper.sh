...
if [ "${SHELLORCHESTRA_DRY_RUN:-0}" = "1" ] || [ "${SHELLORCHESTRA_CONFIRMED:-0}" != "1" ]; then
  printf '{"ok":true,"manager":"zypper","dry_run":true}\n'
  exit 0
fi
shellorchestra_log=$(mktemp)
trap 'rm -f "$shellorchestra_log"' EXIT
run_root zypper --non-interactive dup >>"$shellorchestra_log" 2>&1
...
finish_json zypper "$updated_count" "$updated_packages_json"
